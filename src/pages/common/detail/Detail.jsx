...
function Detail() {
    ...
    if (account?.phone === undefined) {
      ...
      axios
        ...
        .then((res) => {
          ...
          toast.success(
            "Đăng kí thuê phòng thành công! Vui lòng chờ để được chủ nhà xác nhận"
          );
        })
  ...
  const handleUnRentBlog = () => {
    axios
      .put(
        `/api/blog/RentedUnrentRoom/${slug}`,
        {},
        {
          headers: {
            Authorization: `Bearer ${account?.token}`,
  ...
  useEffect(() => {
    ...
    var checkRentConfirm = renterConfirm.findIndex(
      (e) => e === account?.accessToken?.id
    );
    ...
    <div className="detail">
      ...
      <div className="thumnailImage">
        <div className="firstCol">
          <img
            src={`http://${blog?.image[currentImageIndex]}`}
            alt={blog?.image[0]}
          />
        </div>
        {currentImageIndex !== 0 ? (
          <ArrowBackIosRoundedIcon
            className="backIconDetail"
        ...
        <div className="secondCol">
          ...
          <div className="equipment1">
            <i className="boldEquipment">Tên trọ: </i>
            <i className="nameEquipment1">{blog?.title}</i>
          </div>
          <div className="equipment1">
            <i className="boldEquipment">Miêu tả: </i>
            <i className="nameEquipment1">{blog?.description}</i>
          </div>
          <div className="equipment1">
            <i className="boldEquipment">Diện tích: </i>
            <i className="nameEquipment1">{blog?.area} m2</i>
          </div>
          <div className="equipment1">
            <i className="boldEquipment">Đối tượng cho thuê: </i>
            <i className="nameEquipment1">
              {blog?.rentalObject === "male"
                ? "Nam"
                : blog?.rentalObject === "female"
                ? "Nữ"
                : blog?.rentalObject === "both"
                ? "Cả nam và nữ"
                : "Không xác định"}{" "}
            </i>
          </div>
          <div className="equipment1">
            <i className="boldEquipment">Giá tiền: </i>
            <i className="nameEquipment1">
          ...
          <div className="equipment1">
            <i className="boldEquipment">Địa chỉ: </i>
            <i className="nameEquipment1">
              {blog?.addressDetail}, {blog?.ward}, {blog?.district},{" "}
              {blog?.province}
            </i>
          </div>
          ...
          {account?.role === "renter" && isRentRegister === true ? (
            <Button variant="contained" onClick={() => handleClickOpenUnRent()}>
              Huỷ Thuê Trọ
            </Button>
          ) : (
            <></>
          )}
          {(account?.role === "renter" || account?.phone === undefined) &&
          isRentRegister === false &&
          renterId.length !== 2 &&
          isRentConfirm !== true ? (
          ...
          {isRentRegister === false && renterId.length === 2 ? (
          ...
          {account?.role === "renter" && isRentConfirm === true ? (
            <Button variant="contained" disabled>
              Đã Thuê
            </Button>
          ) : (
          ...
          <div className="detailContent">
            ...
            <div className="favouriteRoom">
              ...
              <div className="customerFavourite2">
                <span>
                  Khách hàng đánh giá đây là một trong những ngôi nhà được yêu
                  thích trên HolaHome
                </span>
              </div>
              <div className="customerFavourite3">
                <span>{blog?.avgBlogRate.toFixed(1)}</span>
                <div className="starDetail">
                  <Rating
                    name="hover-feedback"
                    value={blog?.avgBlogRate ?? 5}
                    precision={1}
            ...
            {account?.phone !== undefined ? (
              <div className="lessor">
                ...
                <div className="detailLessor">
                  ...
                  <i className="experience">Số điện thoại: {lessor?.phone}</i>
                </div>
              </div>
            ) : (
              <></>
      ...
      <div className="ratingBlog">
        <div className="numberStar">
          <img
            className="i9if2t0 atm_e2_idpfg4 atm_vy_idpfg4 atm_mk_stnw88 atm_e2_1osqo2v__1lzdix4 atm_vy_1osqo2v__1lzdix4 atm_mk_pfqszd__1lzdix4 i1cqnm0r atm_jp_pyzg9w atm_jr_nyqth1 i1de1kle atm_vh_yfq0k3 dir dir-ltr"
            aria-hidden="true"
            decoding="async"
        ...
        <div className="ratingTitle">
          ...
          <i className="titleDes">
            Một trong những ngôi nhà được yêu thích nhất trên HolaRent dựa trên
            ...
            <div className="bottomDetailLessor">
              <div className="leftBottomDetailLessor">
                ...
                <div className="lessorDescription">
                  <p>
                    Tôi là một người địa phương ở Hoà Lạc, đam mê cung cấp cho
                    sinh viên trải nghiệm nhà trọ tuyệt vời nhất trong thời gian
                    sinh viên.
                  </p>
                </div>
              </div>
              <div className="rightBottomDetailLessor">
                ...
                <i>
                  <b>Tỉ lệ phản hồi:</b> 100%
                </i>
                <i>
                  <b>Thời gian phản hồi:</b> trong vòng một giờ
                </i>
                <button
                  className="contactLessor"
